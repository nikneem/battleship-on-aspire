...
- [ ] 1.1 Add `Games.Abstractions` DTOs for creating a game, joining by code, reading lobby state, and reading player-specific game state, including the create-game result data needed for route handoff.
...
- [ ] 3.1 Add command and query handlers that map DTO-driven requests onto the `Game` aggregate and return caller-safe read models, ensuring host creation uses an existing player identity.
...
- [ ] 4.1 Add a create-game route and UI that lets a visitor enter a player name and an optional lobby password from the app.
- [ ] 4.2 Establish the player profile as part of the create-game flow and enable submission only after player setup succeeds.
- [ ] 4.3 Submit the create-game request, then navigate the host to `/games/{game-code}` using the server-generated game code.

## 5. Verification

- [ ] 5.1 Add unit tests for aggregate creation, protected and unprotected joins, phase transitions, fleet validation, shot legality, and terminal states.
- [ ] 5.2 Add API or handler-level tests for DTO mapping and caller-safe read models that do not leak hidden opponent board data.
- [ ] 5.3 Add frontend tests for the create-game flow, including player-name gating and post-create navigation.
- [ ] 5.4 Run the repository build and test commands and fix any issues needed for the new `Games` flow to pass cleanly.

...
| Start the app (background/detached) | `aspire run --detached` |
| Start isolated (worktrees) | `aspire run --detached --isolated` |
| Restart the app | `aspire run --detached` (stops previous automatically) |
...
Use `aspire run --detached` to run the AppHost in the background. When working in a git worktree, use `--isolated` to avoid port conflicts and to prevent sharing user secrets or other local state with other running instances:
...
Relaunching is safe — `aspire run --detached` automatically stops any previous instance. Re-run `aspire run --detached` whenever changes are made to the AppHost project.
